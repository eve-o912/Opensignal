    ...
    return items.map((item) => {
      ...
      return (
        <div key={`${keyPrefix}${item.id}`}>
          {showSection && (
            <p className="text-xs tracking-widest uppercase text-gray-400 px-3 pt-3 pb-1">{item.section}</p>
          )}
          <button
            onClick={() => {
              onNav(item.id)
  ...
  return (
    <>
      ...
      <div className={`md:hidden fixed inset-0 z-40 transition-opacity duration-200 ${mobileOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        ...
        />
        <aside className={`relative h-full w-72 max-w-[88vw] bg-gray-900 border-r border-gray-800 px-2.5 py-4 transition-transform duration-300 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          <div className="flex items-center justify-between px-2 mb-2">
            <p className="text-sm font-semibold text-white">Menu</p>
            <button
              aria-label="Close menu"
              onClick={onCloseMobile}
              className="inline-flex items-center justify-center w-8 h-8 rounded-lg text-gray-400 hover:bg-gray-800"
            >
